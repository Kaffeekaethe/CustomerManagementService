...
public class JdbcCustomerManagementDAO implements CustomerManagementDAO {
	...
	public Customer getCustomer(String username, String password, boolean fullInformation) {
		...
		/*
		 * Je nachdem, ob alle Informationen gewünscht sind (z.B. Anfrage für
		 * eine Buchung, damit man die Daten nicht selbst eingeben muss, wird
		 * nur die Kunden-ID oder alles ausgelesen
		 * 
		...
		if (!fullInformation) {
			...
			try {
				...
				/*
				 * Die SQL-Queries liegen in einem eigenen Ordner und werden
				 * hier ausgelesen
				 */
				getCustomer = IOUtils.toString(classLoader.getResourceAsStream("Queries/select_customer_id"), "UTF-8");

				/*
				 * Verbindung wird aufgebaut und die Parameter für das Statement
				 * gesetzt
				 */
				...
				/*
				 * Durch das Ausführen der Query erhält man ein Resultset
				 * zurück, auf dessen Grundlage nun der Kunde erstellt wird
				 * 
				 * Dieser Ablauf ist für alle SQL Anfragen gleich, wird also
				 * nicht überall nochmal kommentiert
				 */
			...
			try {
				...
				ResultSet customerRS = getCustomerStmt.executeQuery();
				if (customerRS.next()) {
					return new Customer(customerRS.getInt("customer.id"), customerRS.getString("customer.first_name"),
							customerRS.getString("last_name"), customerRS.getString("street"),
							customerRS.getInt("house_number"), customerRS.getInt("post_code"),
							customerRS.getString("city"), customerRS.getString("e_mail_address"),
							customerRS.getString("payment_method"));
	...
	public String insertAccount(Account account) {
		...
		try {
			String insertAccount = IOUtils.toString(classLoader.getResourceAsStream("Queries/insert_account"), "UTF-8");
	...
	public List<CustomerContact> getContactInformation(List<Integer> customerIDs) {
		...
		try {
			String getContacts = IOUtils.toString(classLoader.getResourceAsStream("Queries/select_customer_contacts"),
					"UTF-8");
